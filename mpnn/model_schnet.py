...
class Set2Set(torch.nn.Module):
    ...
    def softmax(self, x, index, num=None):
        x = x -  torch_scatter.scatter_max(x, index, dim=0, dim_size=num)[0][index]
        x = x.exp()
        x = x / (torch_scatter.scatter_add(x, index, dim=0, dim_size=num)[index] + 1e-16)
        return x
    ...
    def forward(self, x, batch_index):
        ...
        for i in range(self.processing_step):
            ...
            r = torch_scatter.scatter_add(a * x, batch_index, dim=0, dim_size=batch_size) #apply attention #shape = batch_size x ...
...
class Net(torch.nn.Module):
    def __init__(self, node_dim=13, edge_dim=5, num_target=8, num_interactions=1, num_gaussians=50):
        ...
        self.distance_expansion = GaussianSmearing(0.0, 10.0, num_gaussians)
        for _ in range(num_interactions):
            block = InteractionBlock(128, num_gaussians,
                                     128, 10.0)
    ...
    def forward(self, node, edge, edge_index, node_index, coupling_index):
        ...
        #make edges for pytorch built-in Schnet
        edge_index_schnet = edge_index.transpose(0, 1)
        ...
        for interaction in self.interactions:
            node = interaction(node, edge_index_schnet, edge_weight, edge_attr)
